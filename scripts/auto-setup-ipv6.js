...
function runPhp(env){
  ...
  const phpFile='<?php\n'
    ...
    +'  $row=$db->get_row("SELECT `key` FROM `options` WHERE `key`=\'$safeK\' LIMIT 1");\n'
    ...
    +'try{ $db->query("CREATE TABLE IF NOT EXISTS `ipv6_servers` (`id` int(11) NOT NULL AUTO_INCREMENT, `name` varchar(100) NOT NULL, `api_url` varchar(255) NOT NULL, `public_ip` varchar(64) NOT NULL DEFAULT \'\', `admin_key` varchar(512) NOT NULL DEFAULT \'\', `webhook_secret` varchar(512) NOT NULL DEFAULT \'\', `location` varchar(100) NOT NULL DEFAULT \'\', `status` tinyint(1) NOT NULL DEFAULT 1, `sort_order` int(11) NOT NULL DEFAULT 0, `created_at` datetime DEFAULT NULL, `updated_at` datetime DEFAULT NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"); }catch(Throwable $e){}\n'
    +'try{ $has=$db->get_list("SHOW COLUMNS FROM `ipv6_servers`"); $c=false; foreach($has as $col) if($col["Field"]==="api_url") $c=true; }catch(Throwable $e){ $c=false; }\n'
    +'if($c){\n'
    +'  $exists=$db->get_row("SELECT id FROM `ipv6_servers` WHERE `api_url`=\'".addslashes($apiUrl)."\' LIMIT 1");\n'
    +'  if(!$exists){ $db->insert("ipv6_servers",["name"=>$publicIp?:$apiUrl,"api_url"=>$apiUrl,"public_ip"=>$publicIp,"admin_key"=>$encAdmin,"webhook_secret"=>$encWebhook,"status"=>1,"sort_order"=>0,"created_at"=>gettime(),"updated_at"=>gettime()]); echo "OK server seeded id=".$db->get_id_insert()."\n"; }\n'
    +'  else echo "OK server exists id=".$exists["id"]."\n";\n'
    +'}\n'
